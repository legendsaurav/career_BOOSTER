...
async function runDevServer() {
  ...
  const vite = await createViteServer({
    plugins: [react()],
    server: { middlewares: true, hmr: { protocol: 'ws', host: 'localhost', port: DEV_PORT } },
  });
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method !== 'GET' && req.method !== 'HEAD') {
      ...
      proxyBody = await new Promise((resolve, reject) => {
        let data = '';
        req.on('data', (chunk) => {
          data += chunk;
        });
    ...
    try {
      ...
      const headers = Object.fromEntries(
        Array.from(viteRes.headers.entries()).filter(
          ([k]) => !['connection', 'transfer-encoding'].includes(k.toLowerCase())
        )
      );
      res.writeHead(viteRes.status, headers);
