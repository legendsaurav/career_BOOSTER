...
export function initAlumniSearch(rootSelector) {
    ...
    const renderResults = (items) => {
        ...
        // Display the number of members found (up to 40)
        const countInfo = document.createElement('div');
        countInfo.className = 'alumni-count-info';
        countInfo.textContent = `Showing ${items.length} alumni member${items.length !== 1 ? 's' : ''} (max 40)`;
    ...
    const doSearch = async (q) => {
        ...
        try {
            const res = await fetch('https://lol-j8ni.onrender.com/api/alumni_search', {
                ...
                body: JSON.stringify({ query: q, max: 40 })
            });
